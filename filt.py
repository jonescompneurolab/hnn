...
from scipy.signal import iirfilter, lfilter, remez, convolve, get_window, butter
import math
from numpy import vstack, hstack, eye, ones, zeros, linalg, \
newaxis, r_, flipud, convolve, matrix, array, vectorize, angle
...
# bandfilt - bandpass filter signal x using filtfilt
# sampr = sampling rate in Hz
# lohz = low frequency cutoff
# hihz = high frequency cutoff
# bord = order for butterworth filter
def bandfilt (x,sampr,lohz,hihz,bord=2):
    fr = [lohz/(sampr/2.0), hihz/(sampr/2.0)]
    [b,a] = butter(bord,fr,btype='band')
    y=filtfilt(b,a,x)
    return y

# bandfiltlist - apply bandpass filter on x using
#  frequencies centered on lfreq +/- fwidths/2
#  return filtered signals as a list
def bandfiltlist (x,sampr,lfreq,fwidths):  
  lxf = zeros((len(lfreq),len(x))); i=0
  for f in lfreq:
    lxf[i,:] = bandfilt(x,sampr,f - fwidths[i]/2.0,f + fwidths[i]/2.0)
    i += 1
  return lxf

#hilb - returns 2 element list with amplitude,phase numpy arrays
# from hilbert transform of x
# before running hilb, should bandpass filter signal with bandfilt
# typical sequence on signal x:
#  xf = bandfilt(x,sampr,lohz,hihz) <- bandpass filter x into xf
#  xfH = hilb(xf) -> xfH[0] = amplitude, xfH[1] = phase
def hilb (x):    
  xsz=x.size
  lxsz=math.log(xsz)/math.log(2)
  if (lxsz!=int(lxsz)): 
    x=x.copy() # to resize must make x a local copy of the externally referenced x
    x.resize(2**(int(lxsz)+1))
  H = hilbert(x)
  H.resize(xsz) # only needed if stretched
  return [abs(H),angle(H)]

# hilblist - apply hilbert transform on x using
#  frequencies centered on lfreq +/- fwidths/2
#  return amplitude and phase for each frequency as 2 separate lists
#  cuts specifies number of seconds to cut off start/end of filtered signal
#  before applying the hilbert transform
def hilblist (x,sampr,lfreq,fwidths,cuts):
  lhamp,lhang = [], []; i = 0;
  cutsamps = int(cuts*sampr); # number of samples to cut off filtered signal
  sz = len(x); # length of original signal
  lhamp,lhang=zeros((len(lfreq), sz-2*cutsamps)), zeros((len(lfreq), sz-2*cutsamps))
  for f in lfreq:
    xf = bandfilt(x,sampr,f - fwidths[i]/2.0, f + fwidths[i]/2.0) # bandpass filter
    hamp,hang = hilb(xf) # hilbert transform
    del xf # free some memory
    lhamp[i,:] = hamp[cutsamps:sz-cutsamps]
    lhang[i,:] = hang[cutsamps:sz-cutsamps]
    del hamp,hang;
    i += 1
  return lhamp,lhang

#gethilbd - returns a dict with amplitude,phase,filtered signal
def gethilbd (x,sampr,lohz,hihz):
    xf = bandfilt(x,sampr,lohz,hihz) # filter the signal    
    xfh = hilb(xf) # do the hilbert transform
    d = {'amp':xfh[0], 'phase':xfh[1], 'filt':xf}
    return d
