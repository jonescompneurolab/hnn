...
tstop = -1; ntrial = 0; scalefctr = 30e3; lfppath = ''; paramf = ''
...
try:
  ddat['lfp'] = np.loadtxt(os.path.join(basedir,'lfp.txt'))
except:
  print('Could not load',lfppath)
  quit()
...
class LFPCanvas (FigureCanvas):
  ...
  def drawLFP (self, fig):
    ...
    for i in [1]:
      yl[0] = min(yl[0],ddat['lfp'][:,i].min())
      yl[1] = max(yl[1],ddat['lfp'][:,i].max())
    ...
    for i,title in zip([1],ltitle):
      ...
      if self.index == 0: ax.plot(ddat['lfp'][:,0],ddat['lfp'][:,i],'w',linewidth=5)
