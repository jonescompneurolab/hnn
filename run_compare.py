...
if saveJsonPsection:
    ...
    for icell, c in enumerate(net.cells):
        try:
            data[icell] = {}
            if not getattr(c, 'list_all', False):
                c.list_all = [c.soma]
            for isec, sec in enumerate(c.list_all):
                name = str(sec.name()).split('.')[-1]
                data[icell][name] = sec.psection()
                for x in remove:
    ...
    with open('hnn_secs_orig.json', 'w') as f:
        json.dump(data, f)
...
if saveJsonConns:
    ...
    with open('hnn_conns_orig.json', 'w') as f:
        json.dump(data, f)
...
if saveInputSpikes:
    import json
    spk = {}
    for k,v in net.ext_list.items():
        spk[k] = []
        for ext in v:
            spk[k].append(list(ext.eventvec))

    for i,v in enumerate(net.extinput_list):
        if i == 0:
            k = 'extinput_proximal'
        else:
            k = 'extinput_distal'
        spk[k] = list(v.eventvec)

    with open('input_spikes_gamma.json', 'w') as f:
        json.dump(spk, f)